...
npx_directory = r'Z:\pipeline_test_out'
...
run_specs = [									
						['obx_np2_sample', '0', '0,0', '0', '0', ['cortex','thalamus','thalamus','cortex'] ]
]
...
# CatGT params
# ------------
run_CatGT = False   # set to False to sort/process previously processed data (e.g. data that has been through supercat)
...
obx_present = True
ni_present = False
ni_obx_extract_string = '-xa=1,0,0,1,2,1690 -xid=1,0,12,0,1690 '
...
for spec in run_specs:
    ...
    if runTPrime:
        ...
        if create_aux_timepoints:
            ...
            if ni_present:            
                SpikeGLX_utils.CreateAuxTimeEvents(spec[0], str(first_gate), catGT_dest, catGT_out_tag, stream='ni')
            if obx_present:
                ...
                for obx_ind in obx_list:
                    SpikeGLX_utils.CreateAuxTimeEvents(spec[0], str(first_gate), catGT_dest, catGT_out_tag, stream=f'obx{obx_ind}')  
